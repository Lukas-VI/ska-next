    ...
    def ask(self, text: str) -> str:        
        ...
        for i in range(len(relative_memory)):
            ...
            if(total_token >= 1024):
                is_full = True
        if is_full:
            relative_memory = relative_memory[:i]   # 截断记忆
        ...
        # 发送消息
        self.chat_memory.messages.append(message)    # 记录对话
        self.history.messages.append(message)        # 记录对话
        messages.extend(self.chat_memory.messages)   # 历史对话      
        while self.brain.llm.get_num_tokens_from_messages(messages) > 4096:# 限制消息长度
            self.cut_memory()
